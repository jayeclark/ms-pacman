...
export class Tile {
    ...
    static at(position) {
        let { row, col, board: { rows, cols, layout } } = position;
        return row.isBetween(0, rows - 1) && col.isBetween(0, cols - 1) ? layout[row].charAt(col) : 'E';
    }
  
    static cornerTypesAt(position) {

        let cornerTypes = { topLeft: '', topRight: '', bottomLeft: '', bottomRight: '' };
        let [cornerTypeOf, { typeOf } , tile] = [cornerTypes, Tile, Tile.adjacentTiles(position)];
        ...
        for (let thisCorner in cornerTypes) {
            ...
            if (typeOf(tile[thisCorner]).isHall()) {
                if (typeOf(tile[inYDirection]).isHall() && typeOf(tile[inXDirection]).isHall()) {
                    cornerTypeOf[thisCorner] = 'outer';
                }
                else if (typeOf(tile[inYDirection]).isWall() && typeOf(tile[inXDirection]).isWall()) {
                    cornerTypeOf[thisCorner] = 'inner';
                }
            } else { cornerTypeOf[thisCorner] = 'none'; }
        }
        return cornerTypes;
    }
 
    static typeOf(tile) {
        return /[\-SPGBX]/.test(tile) ? tile.replace(/[\-SPB]/,'hall').replace('X','wall').replace('G','ghostbox') : 'off-board';
    }
