...
export class Wall extends Element {
  ...
  makeStyle(position) {
    const [{ board: { tileW } }, { typeOf, cornerTypesAt }] = [position, Tile];
    const style = { top: this.top, 
                    left: this.left,
                    width: (tileW + 1) + 'px',
                    height: (tileW + 1) + 'px'
                  };
    ...
    // Add outer corners
    const outerCorners = Object.entries(cornerTypesAt(position)).filter(([__, v]) => v === 'outer');
    outerCorners.forEach(([k, __]) => { style[camelCase(`border-${k}-radius`)] = (tileW / 2) + "px"; })
  ...
  addInnerCorners(position) {
    ...
    const innerCorners = Object.entries(cornerTypesAt(position)).filter(([__, v]) => v === 'inner');
    for (let [location, __] of innerCorners) {
      const [yDirection, xDirection] = location.split(/(?=[LR])/).map(x=>x.toLowerCase());
      const style = {
                      margin: "-1px", 
                      top: yDirection === 'top' ? (this.top - (tileW / 2) + 3) + 'px' : (this.top + tileW - 2) + 'px',
                      left: xDirection === 'left' ? (this.left - (tileW / 2) + 3) + 'px' : (this.left + tileW - 2) + 'px'
                    };
      const innerCorner = this.makeElement('div', ["inner-corner", `inner-${yDirection}-${xDirection}`], style);
