...
export default class Board {
  ...
  adjustStyleSheet(styleSheet) {
    const {
      tileW: t, cornerW: c, pacDotW: p, fringeW: f,
    } = this;
    const [black, side, sheet] = [
      'rgba(0,0,0,0)',
  ...
  addToGame(dots) {
    ...
    const arrowH = Math.floor((window.innerHeight - parseInt(boardHeight, 10) - 130) / 4);
    const positions = {
      up: parseInt(width, 10) / 2 - arrowH,
      down: parseInt(width, 10) / 2 - arrowH,
      left: parseInt(width, 10) / 2 - arrowH * 2.75,
      right: parseInt(width, 10) / 2 + arrowH * 0.75,
    };
    const rotate = {
      up: '180deg',
      down: '0deg',
