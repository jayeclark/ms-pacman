...
let [dots, count, dCount, powerCount, eatenCount, score] = [
  { dotCount: 0 },
  0,
  0,
  0,
...
String.prototype.isHall = function () {
  return this === "hall";
};
String.prototype.isWall = function () {
  return this === "wall";
};
String.prototype.isOpen = function () {
  return this !== "wall" && this !== "ghostbox";
};
String.prototype.isBlocked = function () {
  return this === "wall" || this === "ghostbox";
};
...
Boolean.prototype.or = function (bool2) {
  return this || bool2;
};
...
function checkCollisions(item) {
  if (item.cache !== "") {
    ...
    const canReverse = ({ cache, direction, board }) =>
      cache === new Directions(board)[direction].reverse;
  ...
  ghosts.forEach((ghost) => {
    ...
    if (ghost.status.mode === "free") {
      ...
      if (right.isBetween(pacL, pacR) || left.isBetween(pacR, pacL)) {
        ghostCollision =
          top.isBetween(pacT, pacB) || bottom.isBetween(pacT, pacB);
      } else if (bottom.isBetween(pacT, pacB) || top.isBetween(pacT, pacB)) {
        ghostCollision =
          left.isBetween(pacL, pacR) || right.isBetween(pacL, pacR);
      }
    ...
    if (powerCount === 0) {
      ghosts.forEach((ghost) => {
        if (ghost.element.style.backgroundColor !== "transparent") {
          ...
          let fringes = Array.from(
            ghost.element.getElementsByClassName("fringe")
          );
          fringes.forEach((fringe) => {
            let { backgroundColor: color, backgroundImage: image } =
              fringe.style;
...
function update() {
  ...
  if (dCount === 9) {
    ...
    bigDots.forEach(
      ({ style }) => (style.display = (!style.display && "none") || null)
    );
    dCount = 0;
  }
...
function updateGhosts() {
  ...
  if (stop === false) {
    ...
    for (let ghost of filteredGhosts) {
      ghost.pickDir();
      ghost.move();
    }
  }
